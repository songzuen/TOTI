	...
	<div class="page-container">
		...
		<div class="demo-content mrg-top-md">
			...
			<div class="container" style="margin-top: -50px">
				<h1 style="color: black">질문 리스트</h1>
				<hr>
				<div id="countdown"></div>
				<div id="questionlistBycate" style="display: none"></div>
				<div id="editFrame" style="display: none">
			        <h1>질문 수정</h1>
			        <hr>
			        <form id="editForm">
			            <input type="hidden" name="quest_idx" id="quest_idx">
					            질문 내용 <input type="text" name="quest_name" id="equest_name" required> <br>
					            답변 방식 <select name="quest_type" id="equest_type">
							<option selected>답변방식을 선택하세요.</option>
				...
				<hr>
				<h1 style="color: black">질문 추가</h1>
				<hr>
				<form id="questionForm">
					<div>
						<label for="cate_idx">분야</label>
				        <select id="cate_idx" name="cate_idx">
						</select>
						<br>
						<label for="cate_idx">답변</label>
					 ...
					 <input type="text" name="quest_name" id="quest_name"
		...
		function categoryOption() {
			$.ajax({
				...
				success : function(data) {
					...
					for (var i = 0; i < data.length; i++) {
						html += '<option value="'+ data[i].cate_idx +'">'+ data[i].cate_name + ' 질문</option>';
					}
					$('#cate_idx').html(html);
				}
			});
